...
if (isset($_POST['create_user'])) {
	...
	if (validerMotDePasse($nouveau_mot_de_passe)) {
		...
		if ($nouveau_mot_de_passe === $confirmation_mot_de_passe) {
			...
			$hashed_password = password_hash($nouveau_mot_de_passe, PASSWORD_DEFAULT); // Hacher le mot de passe
			...
			if ($check_result) {
				...
				if ($user_count == 0) {
					...
					$insert_sql = "INSERT INTO users (user, password,super) VALUES ('$nouvel_utilisateur', '$hashed_password','1')";
				...
				echo "Une erreur s'est produite lors de la vérification de l'existence de l'utilisateur.";
			}
		} else {
...
if (isset($_POST['delete_user'])) {
	...
	if ($user_to_delete !== "") {
		...
		if ($deleteResult) {
			...
			echo "Une erreur s'est produite lors de la suppression de l'utilisateur '$user_to_delete'.";
		}
	}
}
