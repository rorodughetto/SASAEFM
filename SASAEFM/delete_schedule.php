...
if ($result) {
    $reservationInfo = $result->fetch_assoc();
    $startTime = date('d M Y', strtotime($reservationInfo['start_datetime']));
    $endTime = date('d M Y', strtotime($reservationInfo['end_datetime']));

    $room = $reservationInfo['room_name'];
    $name = $reservationInfo['deceased_name'];
    $author = $reservationInfo['author'];
    ...
    if ($_SESSION['utilisateur'] === $author || $rowSuper['super'] == 0) {
        ...
        if ($result_user) {
            ...
            // echo "<br>ID utilisateur : " . $result_user;
            ...
            try {
                ...
                $mail->Body = "Bonjour $author,<br><br>Votre réservation a été supprimée avec succès.<br><br>Informations de réservation :<br> - Nom de la salle : $room<br> - Date de début : $startTime<br> - Date de fin : $endTime<br> - Nom du défunt : $name";
                ...
                echo " <script>alert('Une erreur s'est produite lors de la suppression de la réservation. Erreur d'envoi : " . $mail->ErrorInfo . "'); location.replace('./reservation_salle.php'); </script>";
            }
            try {
                // Propriétés de l'e-mail pour l'utilisateur
                $mail->setFrom('sasaefm@sasaefm.fr', 'SASAEFM');
                ...
                $mail->isHTML(true);
                $mail->Subject = 'Suppression de réservation';
                $mail->Body = "Bonjour,<br><br>La réservation de $author a été supprimée avec succès.<br><br>Informations de réservation :<br> - Nom de la salle : $room<br> - Date de début : $startTime<br> - Date de fin : $endTime<br> - Nom du défunt : $name";
                $mail->AltBody = 'Texte alternatif au format texte brut.';
        ...
        if ($deleteResult) {
            ...
            echo "<script> alert('Réservation supprimée avec succès.');;location.replace('./reservation_salle.php');</script>  </script>";
        } else {
            // En cas d'erreur lors de la suppression
            echo "<script> alert('Une erreur s\'est produite lors de la suppression de la réservation.'); location.replace('./reservation_salle.php');</script> </script>";
        }
    } else {
        // L'utilisateur en cours n'est pas autorisé à supprimer cette réservation
        echo "<script> alert('Vous n\'êtes pas autorisé à supprimer cette réservation. ');location.replace('./reservation_salle.php');</script> </script>";
    }
} else {
    // En cas d'erreur lors de la récupération de l'auteur de la réservation
    echo "<script> alert('Une erreur s\'est produite lors de la récupération des données de réservation.'); location.replace('./reservation_salle.php');</script> </script>";
}
